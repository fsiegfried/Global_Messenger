...
using GlobalMessenger.ViewModel;
using System;
...
namespace GlobalMessenger.Controllers
{
	public class AccountController : SecurityControllerBase
    {
	    ...
	    public async Task<ActionResult> LogOnAsync(LogOnViewModel model)
	    {
		...
		{
			var userId = UserIdentity.GetUserId(this);
			var user = _context.User.FirstOrDefault(u => u.Id == userId && u.Enabled && u.EmailConfirmed && u.Approved);
		...
		public async Task<ActionResult> RecoverAsync(RecoverViewModel model)
        {
			...
			if (ModelState.IsValid)
            {
                var user = _context.User.SingleOrDefault(u => u.UserName == model.UserName && u.Enabled && u.EmailConfirmed && u.Approved);
				if (_configuration.HasRecaptcha)
                {
        ...
        {
            var user = _context.User.SingleOrDefault(u => u.Id.Equals(recoverPasswordModel.Id));
		...
		public ActionResult Chat()
        {
            ...
            return View(new ChatViewModel(user.FirstName, lastAccountActivity, currentUserId,user.PasswordHash));
        }
