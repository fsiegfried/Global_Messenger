	...
	public class UserStore<TUser> : IAppUserStore<User>
	{
		...
		public async Task<LogonResult> TryLogOnAsync(string userName, string password)
		{
			...
			var user = await _context.User
				.SingleOrDefaultAsync(u => u.UserName == userName && u.Enabled && u.Approved && u.EmailConfirmed)
		...
		public async Task<IdentityResult> ChangePasswordFromTokenAsync(string userId, string passwordResetToken, string newPassword)
		{
		...
		public async Task<IdentityResult> ResetPasswordAsync(string userId, string newPassword, string actioningUserName)
		{
