...
namespace GlobalMessenger.Core.Identity
{
	...
	public class AppUserManager : IUserManager, IDisposable
	{
		...
		public async Task<string> LogOnAsync(string userName, bool isPersistent)
		{
			var user = await _userStore.FindByNameAsync(userName);
		...
		public async Task<SeIdentityResult> ChangePasswordFromTokenAsync(string userId, string token, string newPassword)
		{
		...
		public async Task<SeIdentityResult> ChangePasswordAsync(string userId, string oldPassword, string newPassword)
		{
			var user = await FindUserByIdAsync(userId);
			string decryptedSecurityAnswer;
			_encryption.Decrypt(_configuration.EncryptionPassword, user.SecurityAnswerSalt, _configuration.EncryptionIterationCount, user.SecurityAnswer, out decryptedSecurityAnswer);
		...
		public async Task<SeIdentityResult> ResetPasswordAsync(string userId, string actioningUserName)
		{
			var user = await FindUserByIdAsync(userId);
